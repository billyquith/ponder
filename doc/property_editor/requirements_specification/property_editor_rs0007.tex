...
\reqgroup{FILTERING\_GROUPING}
\req{The property editor must display all properties of a CAMP object.}
\req{
Moreover, it must be possible to filter the properties according to their names or their tag names
...
\req{
The CAMP property editor must provide a way to mark, programatically, a property as clean or dirty.
Dirty properties must be emphasized. This way, the user can easily see what is dirty and what is not.
}
...
\req{The property value is displayed and edited with a check box.}
...
\reqgroup{PROPERTY\_INTEGER}
\req{
The property name area must display the property name as a simple text. If the property is dirty,
the text must be bolded.
}

\req{The string representation of the integer must be used as the property value area viewer.}

\req{The property area editor is a spin box for integer number. The value must be changed each time
an arrow is pressed.}
...
\reqgroup{PROPERTY\_REAL}
\req{
The property name area must display the property name as a simple text. If the property is dirty,
the text must be bolded.
}

\req{The string representation of the real number must be used as the property value area viewer.}
...
\reqgroup{PROPERTY\_STRING}
\req{
The property name area must display the property name as a simple text. If the property is dirty,
the text must be bolded.
}

\req{The editor is a line edit.}
...
\req{
The property value area displays the string representation of the object if it exists (like each
node with children in figure \ref{fig:without-classes}).
}

\req{
There is no editor. Instead, the properties of the object are listed in a sub-tree as usual (see
figure \ref{fig:without-classes}).
}

\section{Array}

\reqgroup{PROPERTY\_ARRAY}
\req{
...
\reqgroup{PROPERTY\_ARRAY\_SIMPLE}
...
\reqgroup{PROPERTY\_ARRAY\_OBJECT}
...
\reqgroup{PROPERTY\_ARRAY\_SUBARRAY}
...
\reqgroup{PROPERTY\_ARRAY\_DROPDOWNLIST}
...
\reqgroup{PROPERTY\_ARRAY\_DYNAMIC}

Dynamic arrays allow to insert and remove elements. The property editor must provide an access to
these features.

\req{
For dynamic arrays in collapsed mode, the drop-down list must display:
\begin{itemize}
...
one must allow to remove the clicked element. An additional node at the end of the subtree must
contain a push button to insert an element at the end of the array.
}
